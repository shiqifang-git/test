package com.shineyue.bean.ztbg;

public class Jcdw_jcrdwtzBean {
	private double zyje;
	private String xingming;
	private String zjhm;
	private String ywbm;
	
	public String getYwbm() {
		return ywbm;
	}
	public void setYwbm(String ywbm) {
		this.ywbm = ywbm;
	}
	public double getZyje() {
		return zyje;
	}
	public void setZyje(double zyje) {
		this.zyje = zyje;
	}
	public String getXingming() {
		return xingming;
	}
	public void setXingming(String xingming) {
		this.xingming = xingming;
	}
	public String getZjhm() {
		return zjhm;
	}
	public void setZjhm(String zjhm) {
		this.zjhm = zjhm;
	}
	private String hbjywlsh;//汇补缴业务流水号varchar(20)
	private String hjrq;//汇缴日期date
	private int syrs;//上月汇缴人数int
	private double syje;//上月汇缴金额decimal(12,2)
	private int zjrs;//本月增加人数int
	private String bgrq;
	private String jbrzjhm;
	private String grzhzt;
	private String bgqgrzhzt;
	private String bghgrzhzt;
	private String zydwzh;
	private String zydwmc;
	private int rs;
	private double zcjce;
	private String lrczy;
	private String spczy;
	private String zxmc;
	private String username;
	
	public String getZxmc() {
		return zxmc;
	}
	public void setZxmc(String zxmc) {
		this.zxmc = zxmc;
	}
	public String getUsername() {
		return username;
	}
	public void setUsername(String username) {
		this.username = username;
	}
	public String getSpczy() {
		return spczy;
	}
	public void setSpczy(String spczy) {
		this.spczy = spczy;
	}
	public String getLrczy() {
		return lrczy;
	}
	public void setLrczy(String lrczy) {
		this.lrczy = lrczy;
	}
	public String getZydwzh() {
		return zydwzh;
	}
	public void setZydwzh(String zydwzh) {
		this.zydwzh = zydwzh;
	}
	public String getZydwmc() {
		return zydwmc;
	}
	public void setZydwmc(String zydwmc) {
		this.zydwmc = zydwmc;
	}
	public int getRs() {
		return rs;
	}
	public void setRs(int rs) {
		this.rs = rs;
	}
	public double getZcjce() {
		return zcjce;
	}
	public void setZcjce(double zcjce) {
		this.zcjce = zcjce;
	}
	public String getBgrq() {
		return bgrq;
	}
	public void setBgrq(String bgrq) {
		this.bgrq = bgrq;
	}
	public String getJbrzjhm() {
		return jbrzjhm;
	}
	public void setJbrzjhm(String jbrzjhm) {
		this.jbrzjhm = jbrzjhm;
	}
	public String getGrzhzt() {
		return grzhzt;
	}
	public void setGrzhzt(String grzhzt) {
		this.grzhzt = grzhzt;
	}
	public String getBgqgrzhzt() {
		return bgqgrzhzt;
	}
	public void setBgqgrzhzt(String bgqgrzhzt) {
		this.bgqgrzhzt = bgqgrzhzt;
	}
	public String getBghgrzhzt() {
		return bghgrzhzt;
	}
	public void setBghgrzhzt(String bghgrzhzt) {
		this.bghgrzhzt = bghgrzhzt;
	}
	public String getHbjywlsh() {
		return hbjywlsh;
	}
	public void setHbjywlsh(String hbjywlsh) {
		this.hbjywlsh = hbjywlsh;
	}
	public String getHjrq() {
		return hjrq;
	}
	public void setHjrq(String hjrq) {
		this.hjrq = hjrq;
	}
	public int getSyrs() {
		return syrs;
	}
	public void setSyrs(int syrs) {
		this.syrs = syrs;
	}
	public double getSyje() {
		return syje;
	}
	public void setSyje(double syje) {
		this.syje = syje;
	}
	public int getZjrs() {
		return zjrs;
	}
	public void setZjrs(int zjrs) {
		this.zjrs = zjrs;
	}
	public double getZjje() {
		return zjje;
	}
	public void setZjje(double zjje) {
		this.zjje = zjje;
	}
	public int getJsrs() {
		return jsrs;
	}
	public void setJsrs(int jsrs) {
		this.jsrs = jsrs;
	}
	public double getJsje() {
		return jsje;
	}
	public void setJsje(double jsje) {
		this.jsje = jsje;
	}
	public int getHjrs() {
		return hjrs;
	}
	public void setHjrs(int hjrs) {
		this.hjrs = hjrs;
	}
	public double getHjje() {
		return hjje;
	}
	public void setHjje(double hjje) {
		this.hjje = hjje;
	}
	public double getBjje() {
		return bjje;
	}
	public void setBjje(double bjje) {
		this.bjje = bjje;
	}
	public int getBjrs() {
		return bjrs;
	}
	public void setBjrs(int bjrs) {
		this.bjrs = bjrs;
	}
	public String getJkczy() {
		return jkczy;
	}
	public void setJkczy(String jkczy) {
		this.jkczy = jkczy;
	}
	public String getKsyd() {
		return ksyd;
	}
	public void setKsyd(String ksyd) {
		this.ksyd = ksyd;
	}
	public String getJsyd() {
		return jsyd;
	}
	public void setJsyd(String jsyd) {
		this.jsyd = jsyd;
	}
	private double zjje;//本月增加金额decimal(12,2)
	private int jsrs;//本月减少人数int
	private double jsje;//本月减少金额decimal(12,2)
	private int hjrs;//本月汇缴人数int
	private double hjje;//本月汇缴金额decimal(12,2)
	private double bjje;//补缴金额decimal(12,2)
	private int bjrs;//补缴人数int
	private String jkczy;//缴款操作员varchar(100)
	private String ksyd;//开始月度varchar(6)
	private String jsyd;//结束月底varchar(6)
	private String ffbm;//服务分类编码varchar(2)
	private String zxbm;//中心编码varchar(2)
	private String jgbm;//机构编码varchar(50)
	private String khbh;//客户编号varchar(12)
	private String zhbh;//账户编号varchar(40)
	private String ywfl;//业务分类varchar(20)
	private String ywlb;//业务类别varchar(2)
	private String ywlsh;//业务流水号varchar(20)
	private String blqd;//办理渠道varchar(20)
	private String userid;//操作员代码varchar(20)
	private int totalcount;//返回记录数int
	private double jkje;//缴款金额decimal(12,2)
	public String getFfbm() {
		return ffbm;
	}
	public void setFfbm(String ffbm) {
		this.ffbm = ffbm;
	}
	public String getZxbm() {
		return zxbm;
	}
	public void setZxbm(String zxbm) {
		this.zxbm = zxbm;
	}
	public String getJgbm() {
		return jgbm;
	}
	public void setJgbm(String jgbm) {
		this.jgbm = jgbm;
	}
	public String getKhbh() {
		return khbh;
	}
	public void setKhbh(String khbh) {
		this.khbh = khbh;
	}
	public String getZhbh() {
		return zhbh;
	}
	public void setZhbh(String zhbh) {
		this.zhbh = zhbh;
	}
	public String getYwfl() {
		return ywfl;
	}
	public void setYwfl(String ywfl) {
		this.ywfl = ywfl;
	}
	public String getYwlb() {
		return ywlb;
	}
	public void setYwlb(String ywlb) {
		this.ywlb = ywlb;
	}
	public String getYwlsh() {
		return ywlsh;
	}
	public void setYwlsh(String ywlsh) {
		this.ywlsh = ywlsh;
	}
	public String getBlqd() {
		return blqd;
	}
	public void setBlqd(String blqd) {
		this.blqd = blqd;
	}
	public String getUserid() {
		return userid;
	}
	public void setUserid(String userid) {
		this.userid = userid;
	}
	public int getTotalcount() {
		return totalcount;
	}
	public void setTotalcount(int totalcount) {
		this.totalcount = totalcount;
	}
	public double getJkje() {
		return jkje;
	}
	public void setJkje(double jkje) {
		this.jkje = jkje;
	}
	public String getYwrq() {
		return ywrq;
	}
	public void setYwrq(String ywrq) {
		this.ywrq = ywrq;
	}
	public String getYwzy() {
		return ywzy;
	}
	public void setYwzy(String ywzy) {
		this.ywzy = ywzy;
	}
	public String getYwfhczy() {
		return ywfhczy;
	}
	public void setYwfhczy(String ywfhczy) {
		this.ywfhczy = ywfhczy;
	}
	public String getDwzh() {
		return dwzh;
	}
	public void setDwzh(String dwzh) {
		this.dwzh = dwzh;
	}
	public String getDwmc() {
		return dwmc;
	}
	public void setDwmc(String dwmc) {
		this.dwmc = dwmc;
	}
	public String getJbrxm() {
		return jbrxm;
	}
	public void setJbrxm(String jbrxm) {
		this.jbrxm = jbrxm;
	}
	public String getJbrsjhm() {
		return jbrsjhm;
	}
	public void setJbrsjhm(String jbrsjhm) {
		this.jbrsjhm = jbrsjhm;
	}
	public int getDwjcrs() {
		return dwjcrs;
	}
	public void setDwjcrs(int dwjcrs) {
		this.dwjcrs = dwjcrs;
	}
	public double getYjcze() {
		return yjcze;
	}
	public void setYjcze(double yjcze) {
		this.yjcze = yjcze;
	}
	public String getStyhmc() {
		return styhmc;
	}
	public void setStyhmc(String styhmc) {
		this.styhmc = styhmc;
	}
	public int getSize() {
		return size;
	}
	public void setSize(int size) {
		this.size = size;
	}
	public int getPage() {
		return page;
	}
	public void setPage(int page) {
		this.page = page;
	}
	private String ywrq;//业务日期date
	private String ywzy;//业务摘要varchar(200)
	private String ywfhczy;//业务复核操作员varchar(100)
	private String dwzh;//单位账号varchar(20)
	private String dwmc;//单位名称varchar(255)
	private String jbrxm;//经办人姓名varchar(100)
	private String jbrsjhm;//经办人手机号码varchar(40)
	private int dwjcrs;//单位缴存人数int
	private double yjcze;//月缴存总额decimal(12,2)
	private String styhmc;//受托银行名称varchar(100)
	private int size;//int
	private int page;//int
}
