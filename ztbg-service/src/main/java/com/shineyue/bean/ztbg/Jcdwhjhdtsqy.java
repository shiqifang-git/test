package com.shineyue.bean.ztbg;

public class Jcdwhjhdtsqy {
	private String zjbzxbm;//住建部中心编码
	private String jbzhyhdm;
	private String lxdh;
	private String id;
	private String sksszh;
	private String skyhmc;
	private String wtskxyh;
	private String batchprjno;
	private String grbh;
	private String yhbm2;
	private String ywzy;
	private String pcid;
	private String jysj;
	private double tsjehj;
	private String fksszh;
	private String dwbh;
	private String yhmc;
	private String qyztmc;
	private String bgqydtsrq;
	private String bgqqyrlxdh;
	private String bgqqyzt;
	private String fszt;
	private String hbjksny;
	private String hbjjsny;
	private String lrrq;
	
	
	public String getBgqydtsrq() {
    	return bgqydtsrq;
    }
	public void setBgqydtsrq(String bgqydtsrq) {
    	this.bgqydtsrq = bgqydtsrq;
    }
	public String getBgqqyrlxdh() {
    	return bgqqyrlxdh;
    }
	public void setBgqqyrlxdh(String bgqqyrlxdh) {
    	this.bgqqyrlxdh = bgqqyrlxdh;
    }
	public String getBgqqyzt() {
    	return bgqqyzt;
    }
	public void setBgqqyzt(String bgqqyzt) {
    	this.bgqqyzt = bgqqyzt;
    }
	public String getYhmc() {
		return yhmc;
	}
	public void setYhmc(String yhmc) {
		this.yhmc = yhmc;
	}
	public String getQyztmc() {
		return qyztmc;
	}
	public void setQyztmc(String qyztmc) {
		this.qyztmc = qyztmc;
	}
	public String getDwbh() {
		return dwbh;
	}
	public void setDwbh(String dwbh) {
		this.dwbh = dwbh;
	}
	public String getFksszh() {
		return fksszh;
	}
	public void setFksszh(String fksszh) {
		this.fksszh = fksszh;
	}
	public double getTsjehj() {
		return tsjehj;
	}
	public void setTsjehj(double tsjehj) {
		this.tsjehj = tsjehj;
	}
	public String getPcid() {
		return pcid;
	}
	public void setPcid(String pcid) {
		this.pcid = pcid;
	}
	public String getJysj() {
		return jysj;
	}
	public void setJysj(String jysj) {
		this.jysj = jysj;
	}
	public String getYwzy() {
		return ywzy;
	}
	public void setYwzy(String ywzy) {
		this.ywzy = ywzy;
	}
	public String getYhbm2() {
		return yhbm2;
	}
	public void setYhbm2(String yhbm2) {
		this.yhbm2 = yhbm2;
	}
	public String getGrbh() {
		return grbh;
	}
	public void setGrbh(String grbh) {
		this.grbh = grbh;
	}
	public String getBatchprjno() {
		return batchprjno;
	}
	public void setBatchprjno(String batchprjno) {
		this.batchprjno = batchprjno;
	}
	public String getWtskxyh() {
		return wtskxyh;
	}
	public void setWtskxyh(String wtskxyh) {
		this.wtskxyh = wtskxyh;
	}
	public String getSkyhmc() {
		return skyhmc;
	}
	public void setSkyhmc(String skyhmc) {
		this.skyhmc = skyhmc;
	}
	public String getSksszh() {
		return sksszh;
	}
	public void setSksszh(String sksszh) {
		this.sksszh = sksszh;
	}
	public String getId() {
		return id;
	}
	public void setId(String id) {
		this.id = id;
	}
	public String getLxdh() {
		return lxdh;
	}
	public void setLxdh(String lxdh) {
		this.lxdh = lxdh;
	}
	public String getZjbzxbm() {
		return zjbzxbm;
	}
	public void setZjbzxbm(String zjbzxbm) {
		this.zjbzxbm = zjbzxbm;
	}
	public String getJbzhyhdm() {
		return jbzhyhdm;
	}
	public void setJbzhyhdm(String jbzhyhdm) {
		this.jbzhyhdm = jbzhyhdm;
	}
	private double tsje;
	public double getTsje() {
		return tsje;
	}
	public void setTsje(double tsje) {
		this.tsje = tsje;
	}
	public String getJcyd() {
		return jcyd;
	}
	public void setJcyd(String jcyd) {
		this.jcyd = jcyd;
	}
	public String getSbyy() {
		return sbyy;
	}
	public void setSbyy(String sbyy) {
		this.sbyy = sbyy;
	}
	private String jcyd;
	private String sbyy;
	private String xingming;
	public String getXingming() {
		return xingming;
	}
	public void setXingming(String xingming) {
		this.xingming = xingming;
	}
	public String getZjhm() {
		return zjhm;
	}
	public void setZjhm(String zjhm) {
		this.zjhm = zjhm;
	}
	private double yjcze;//月缴存总额
	private String jbrxm;//经办人姓名
	private String dwmc;//单位名称
	private String dwjcrs;//单位缴存人数
	private String styhmc;//受托银行名称
	private String jbrsjhm;//经办人手机号码
	private String ywjbr;//业务经办人
	private String ywlsh;
	private String zjhm;
	private String grzh;//个人账号
	private String grtsxyh;//委托收款协议号
	private String jbzhyhzh;//委托收款签约银行账号
	private String jbzhkhyh;//委托收款签约开户银行
	private String yhbm;//委托收款签约所属银行编码
	private String qyrq;//签约日期
	private String ydtsrq;//约定托收日期
	private String blqd;//办理渠道
	private int  userid;//操作员id
	private String msg;//返回值
	private String ret;
	private int count;
	private int page;
	private int size;
	private String ywrq;
	private String dqyd;
	private String jyzt;
	private String zxbm;
	private String jgbm;
	private String ywfl;
	private String ywlb;
	private String khbh;
	private String ffbm;
	private String dwzh;//单位账号
	private String wtxyh;//委托协议号
	private String jbzhkhmc;//基本账户开户名称
	private String zhbh;
	private int totalcount;
	private String qyzt;
	private String qyrlxdh;
	private String khyhhh;
	private String jyztbm;
	
	private String sprq;
	private String thrq;
	private String bpmid;
	private String wf_processunid;
	private String wf_docunid;
	private String splb;
	public String getJyztbm() {
		return jyztbm;
	}
	public void setJyztbm(String jyztbm) {
		this.jyztbm = jyztbm;
	}
	public String getQyrlxdh() {
		return qyrlxdh;
	}
	public void setQyrlxdh(String qyrlxdh) {
		this.qyrlxdh = qyrlxdh;
	}
	public String getKhyhhh() {
		return khyhhh;
	}
	public void setKhyhhh(String khyhhh) {
		this.khyhhh = khyhhh;
	}
	public double getYjcze() {
		return yjcze;
	}
	public void setYjcze(double yjcze) {
		this.yjcze = yjcze;
	}
	public String getJbrxm() {
		return jbrxm;
	}
	public void setJbrxm(String jbrxm) {
		this.jbrxm = jbrxm;
	}
	public String getDwmc() {
		return dwmc;
	}
	public void setDwmc(String dwmc) {
		this.dwmc = dwmc;
	}
	public String getDwjcrs() {
		return dwjcrs;
	}
	public void setDwjcrs(String dwjcrs) {
		this.dwjcrs = dwjcrs;
	}
	public String getStyhmc() {
		return styhmc;
	}
	public void setStyhmc(String styhmc) {
		this.styhmc = styhmc;
	}
	public String getJbrsjhm() {
		return jbrsjhm;
	}
	public void setJbrsjhm(String jbrsjhm) {
		this.jbrsjhm = jbrsjhm;
	}
	public String getYwjbr() {
		return ywjbr;
	}
	public void setYwjbr(String ywjbr) {
		this.ywjbr = ywjbr;
	}
	public String getYwlsh() {
		return ywlsh;
	}
	public void setYwlsh(String ywlsh) {
		this.ywlsh = ywlsh;
	}
	public String getDwzh() {
		return dwzh;
	}
	public void setDwzh(String dwzh) {
		this.dwzh = dwzh;
	}
	public String getWtxyh() {
		return wtxyh;
	}
	public void setWtxyh(String wtxyh) {
		this.wtxyh = wtxyh;
	}
	public String getJbzhkhmc() {
		return jbzhkhmc;
	}
	public void setJbzhkhmc(String jbzhkhmc) {
		this.jbzhkhmc = jbzhkhmc;
	}
	public String getZhbh() {
		return zhbh;
	}
	public void setZhbh(String zhbh) {
		this.zhbh = zhbh;
	}
	public int getTotalcount() {
		return totalcount;
	}
	public void setTotalcount(int totalcount) {
		this.totalcount = totalcount;
	}
	public String getQyzt() {
		return qyzt;
	}
	public void setQyzt(String qyzt) {
		this.qyzt = qyzt;
	}
	public String getZxbm() {
		return zxbm;
	}
	public void setZxbm(String zxbm) {
		this.zxbm = zxbm;
	}
	public String getJgbm() {
		return jgbm;
	}
	public void setJgbm(String jgbm) {
		this.jgbm = jgbm;
	}
	public String getYwfl() {
		return ywfl;
	}
	public void setYwfl(String ywfl) {
		this.ywfl = ywfl;
	}
	public String getYwlb() {
		return ywlb;
	}
	public void setYwlb(String ywlb) {
		this.ywlb = ywlb;
	}
	public String getKhbh() {
		return khbh;
	}
	public void setKhbh(String khbh) {
		this.khbh = khbh;
	}
	public String getFfbm() {
		return ffbm;
	}
	public void setFfbm(String ffbm) {
		this.ffbm = ffbm;
	}
	public String getYwrq() {
		return ywrq;
	}
	public void setYwrq(String ywrq) {
		this.ywrq = ywrq;
	}
	public String getDqyd() {
		return dqyd;
	}
	public void setDqyd(String dqyd) {
		this.dqyd = dqyd;
	}
	public String getJyzt() {
		return jyzt;
	}
	public void setJyzt(String jyzt) {
		this.jyzt = jyzt;
	}
	public int getCount() {
		return count;
	}
	public void setCount(int count) {
		this.count = count;
	}
	public int getPage() {
		return page;
	}
	public void setPage(int page) {
		this.page = page;
	}
	public int getSize() {
		return size;
	}
	public void setSize(int size) {
		this.size = size;
	}
	public String getRet() {
		return ret;
	}
	public void setRet(String ret) {
		this.ret = ret;
	}
	public String getGrzh() {
		return grzh;
	}
	public void setGrzh(String grzh) {
		this.grzh = grzh;
	}
	public String getGrtsxyh() {
		return grtsxyh;
	}
	public void setGrtsxyh(String grtsxyh) {
		this.grtsxyh = grtsxyh;
	}
	public String getJbzhyhzh() {
		return jbzhyhzh;
	}
	public void setJbzhyhzh(String jbzhyhzh) {
		this.jbzhyhzh = jbzhyhzh;
	}
	public String getJbzhkhyh() {
		return jbzhkhyh;
	}
	public void setJbzhkhyh(String jbzhkhyh) {
		this.jbzhkhyh = jbzhkhyh;
	}
	public String getYhbm() {
		return yhbm;
	}
	public void setYhbm(String yhbm) {
		this.yhbm = yhbm;
	}
	public String getQyrq() {
		return qyrq;
	}
	public void setQyrq(String qyrq) {
		this.qyrq = qyrq;
	}
	public String getYdtsrq() {
		return ydtsrq;
	}
	public void setYdtsrq(String ydtsrq) {
		this.ydtsrq = ydtsrq;
	}
	public String getBlqd() {
		return blqd;
	}
	public void setBlqd(String blqd) {
		this.blqd = blqd;
	}
	public int getUserid() {
		return userid;
	}
	public void setUserid(int userid) {
		this.userid = userid;
	}
	public String getMsg() {
		return msg;
	}
	public void setMsg(String msg) {
		this.msg = msg;
	}
	public String getSprq() {
		return sprq;
	}
	public void setSprq(String sprq) {
		this.sprq = sprq;
	}
	public String getThrq() {
		return thrq;
	}
	public void setThrq(String thrq) {
		this.thrq = thrq;
	}
	public String getBpmid() {
		return bpmid;
	}
	public void setBpmid(String bpmid) {
		this.bpmid = bpmid;
	}
	public String getWf_processunid() {
		return wf_processunid;
	}
	public void setWf_processunid(String wf_processunid) {
		this.wf_processunid = wf_processunid;
	}
	public String getWf_docunid() {
		return wf_docunid;
	}
	public void setWf_docunid(String wf_docunid) {
		this.wf_docunid = wf_docunid;
	}
	public String getSplb() {
		return splb;
	}
	public void setSplb(String splb) {
		this.splb = splb;
	}
	public String getFszt() {
		return fszt;
	}
	public void setFszt(String fszt) {
		this.fszt = fszt;
	}
	public String getHbjksny() {
		return hbjksny;
	}
	public void setHbjksny(String hbjksny) {
		this.hbjksny = hbjksny;
	}
	public String getHbjjsny() {
		return hbjjsny;
	}
	public void setHbjjsny(String hbjjsny) {
		this.hbjjsny = hbjjsny;
	}
	public String getLrrq() {
		return lrrq;
	}
	public void setLrrq(String lrrq) {
		this.lrrq = lrrq;
	}
}
